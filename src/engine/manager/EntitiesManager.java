...
public class EntitiesManager 
{
	...
	public void cleanList() {
		...
		if(removeRessources.isEmpty() == false) {
			...
			waitingToAdd.removeAll(removeRessources);
			removeList.removeAll(removeRessources);
		...
		if(removeStorageBuildings.isEmpty() == false) {
			...
			waitingToAdd.removeAll(removeStorageBuildings);
			removeList.removeAll(removeStorageBuildings);
		...
		if(removeAttackBuildings.isEmpty() == false) {
			...
			waitingToAdd.removeAll(removeAttackBuildings);
			removeList.removeAll(removeAttackBuildings);
		...
		if(removeProdBuildings.isEmpty() == false) {
			...
			waitingToAdd.removeAll(removeProdBuildings);
			removeList.removeAll(removeProdBuildings);
		...
		if(removeWorkers.isEmpty() == false) {
			...
			waitingToAdd.removeAll(removeWorkers);
			removeList.removeAll(removeWorkers);
		...
		if(removeFighters.isEmpty() == false) {
			...
			waitingToAdd.removeAll(removeFighters);
			removeList.removeAll(removeFighters);
		...
		if(removeSiteConstructions.isEmpty() == false) {
			...
			waitingToAdd.removeAll(removeSiteConstructions);
			removeList.removeAll(removeSiteConstructions);
	...
	public void createFighter(int id, int faction, ForFighter patron, Position position, Position destination)
	{
		...
		if(id == EntityConfiguration.INFANTRY) {
			fighter = new Fighter(patron.getHp(), 0, patron.getAttackRange(), patron.getAttackSpeed(), patron.getMaxSpeed(), patron.getDamage(), patron.getRange(), patron.getArmor(), 0, position, id, patron.getDescription(), patron.getHpMax(), faction, destination, patron.getSightRange(), 4, graphicsManager);
		}
